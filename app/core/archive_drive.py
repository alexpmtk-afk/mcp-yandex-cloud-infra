"""Google Drive storage backend for the durable marketplace archive.

The remote MCP must own its Google Drive session so archive updates work from
any ChatGPT/Codex client. We use a long-lived OAuth refresh token kept in
Lockbox; access tokens exist only in process memory and are refreshed on demand.
"""
...
def build_drive_archive_store_from_env() -> GoogleDriveArchiveStore | None:
    try:
