    ...
    with tempfile.TemporaryDirectory(prefix="marketplace-archive-") as tmp:
        ...
        try:
            for cabinet in ARCHIVE_CABINETS:
                ...
                conn.execute(
                    f"CREATE VIEW {cabinet} AS SELECT *, '{cabinet}' AS archive_cabinet "
                    f"FROM read_csv_auto('{escaped}', delim=';', header=true, all_varchar=true, union_by_name=true, ignore_errors=false)"
                )
            ...
            conn.execute(f"CREATE VIEW wb_all AS {' UNION ALL BY NAME '.join(f'SELECT * FROM {name}' for name in available)}")
...
def register_archive_tools(mcp: FastMCP, modules: dict[str, Any], store: Any | None) -> None:
    ...
    @mcp.tool(name="marketplace_archive_update", annotations={"title": "Queue central marketplace database archive update", "readOnlyHint": False, "openWorldHint": True})
    async def marketplace_archive_update(year: int = date.today().year, seller: str = "all", max_reports_per_cabinet: int = 4) -> str:
        ...
        return _j({"ok": True, "marketplace": "wb", "dataset": "wb_weekly_finance_main", "year": int(year), "queued": True, "jobs": jobs, "instruction": "Process queued jobs with marketplace_archive_worker_step; no long quota wait occurs inside MCP."})
    ...
    async def marketplace_archive_worker_step(job_id: str = "") -> str:
        ...
        Provider download stages perform at most one WB API request. Large annual-file writes use one Google Drive resumable-upload chunk per step.
        """
        if store is None:
            return _not_configured()
        queue = WBFinanceArchiveJobQueue(wb, store)
