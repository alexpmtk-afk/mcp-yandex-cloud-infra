...
async function api(path,opt={}){opt.headers={...(opt.headers||{}),'X-Admin-Token':token,'Content-Type':'application/json'};const r=await fetch(path,opt);if(r.status===401)throw new Error('Неверный ключ управления');if(!r.ok)throw new Error((await r.text())||('HTTP '+r.status));return r.json()}
