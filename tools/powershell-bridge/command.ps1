# MANAGER_MP2_DIAGNOSTIC_ASCII=YES
...
foreach ($r in @('X:\','G:\','D:\','C:\')) {
  ...
  try {
    $hit = Get-ChildItem -LiteralPath $r -Directory -Recurse -Depth 7 -ErrorAction SilentlyContinue |
...
if ($profilePath) {
  foreach ($p in @(
    ...
    Write-Host "CHECK_PATH=$p EXISTS=$(Test-Path -LiteralPath $p)"
  }
}
